...
public class Verify {
  ...
  public void randomTrees( int _noNodes, int _nTrees ) {
	...
	if ( AVL.get(randKey) == null ) { // Needs change !!!
	  // insert into AVL
	  AVL.put(randKey, null);
	  // insert into BST
	  BST.put(randKey, null);
	  // next key
	  ++count;
	}
      } while ( count < _noNodes );
      // updateCounters( height( treeAVL ), height( treeBST));
      updateCounters( getheight(AVL), getheight(BST)); // Needs change !!!
    }
    return;
  }
  ...
  public <T> int height( BinaryTree<T> bTree ) {
    return bTree.getheight(); // change to return the height of bTree
  }
